"""
FaceDetector v5.10 - MediaPipe FaceLandmarker wrapper.
Enhanced: Eye openness check, BBox jerk detection, Landmark stability gating,
          Head Pose 6DoF (cv2.solvePnP).
"""
...
class FaceData:
    ...
    def head_pose_6dof(self):
        """Ước lượng hướng mặt 6DoF chính xác bằng cv2.solvePnP().

        Returns:
            dict: {'yaw': float, 'pitch': float, 'roll': float, 'success': bool}
                  Đơn vị: độ (degree).
                  yaw dương = quay phải, pitch dương = ngẩng lên, roll dương = nghiêng phải.
        """
        # Local instantiation để đảm bảo 100% thread-safe
        estimator = HeadPoseEstimator(self.img_w, self.img_h)
        return estimator.estimate(self.lm2d)

    def head_pose(self):
        """Get head pose offsets (h_offset, v_ratio) — backward compatible.
        Nội bộ dùng 6DoF nếu có, fallback heuristic nếu solvePnP fail.
        """
        # Thử 6DoF trước
        try:
            pose = self.head_pose_6dof()
            if pose['success']:
                # Chuyển yaw/pitch → h_offset/v_ratio tương đương API cũ
                # yaw 30° ≈ h_off 0.22 (tỉ lệ cũ)
                h_off = pose['yaw'] / 135.0   # normalize ~[-1, 1] 
                v_ratio = 0.40 - pose['pitch'] / 90.0  # normalize quanh 0.40 (mặt thẳng)
    ...
    def check_pose(self, direction):
        ...
        Dùng 6DoF angles nếu có, fallback heuristic.
        """
        if direction == "any":
            return True, "OK"

        # Thử 6DoF trước (chính xác ±3°)
        try:
            pose = self.head_pose_6dof()
            if pose['success']:
                ok, msg, _ = check_pose_6dof(pose, direction)
                return ok, msg
        except Exception:
            pass
